...
const CHOOSE_WORD_DURATION_MS = 20_000;
...
router.post("/:roomId/guess", (req, res) => {
  ...
  if (!isCorrectGuess) {
    ...
    if (levenshteinDistance(normalizedGuess, normalizedWord) <= 2) {
      appendMessage(
        room,
        buildMessage("system", "System", "You're close! Only 2 letters off.", { privateTo: resolvedPlayer })
      );
    }
  } else {
